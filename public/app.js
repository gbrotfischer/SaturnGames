...
const dom = {
  ...
  storeTotalGames: document.getElementById('store-total-games'),
...
async function loadGames(force = false) {
  if (!supabase) {
    if (dom.storeTotalGames) dom.storeTotalGames.textContent = '0';
  ...
  gamesLoadingPromise = (async () => {
    ...
    if (dom.storeTotalGames) {
      dom.storeTotalGames.textContent = gamesCache.length.toString();
    }
